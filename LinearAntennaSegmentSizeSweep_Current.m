%% sweep for Current Distribution * 5 size 
clear all;close all;
Frequency=1e9;
segment_point_fac30 = 7;
...
Ny_List=10+(0:segment_point_fac30-1)*30;
Zin_NumericalResultList=zeros(1,segment_point_fac30);

TotalSegmentsList=cell(1,segment_point_fac30);
CurrentInterpList=cell(1,segment_point_fac30);
for seg_id=1:length(Ny_List)
    ...
    TotalSegments=(Ny-1);
    TotalSegmentsList{seg_id}=TotalSegments;
    ...
    % loop all "source" triangles
        ...
        for n_tri = 1:TotalElements % 三角形段数，Gibson的加权基函数个数N
    ...
    CurrentInterpList{seg_id}=CurrentInterp;
end
%%
figure(2);
subplot(121)
...
plot(line_length*(1:TotalSegmentsList{7}*NumGauss)/(TotalSegmentsList{7}*NumGauss),abs(CurrentInterpList{7}),'-y','Linewidth',3);
plot(line_length*(1:TotalSegmentsList{1}*NumGauss)/(TotalSegmentsList{1}*NumGauss),abs(CurrentInterpList{1}),'-r','Linewidth',3);
plot(line_length*(1:TotalSegmentsList{2}*NumGauss)/(TotalSegmentsList{2}*NumGauss),abs(CurrentInterpList{2}),'-g','Linewidth',3);
plot(line_length*(1:TotalSegmentsList{3}*NumGauss)/(TotalSegmentsList{3}*NumGauss),abs(CurrentInterpList{3}),'-b','Linewidth',3);
plot(line_length*(1:TotalSegmentsList{4}*NumGauss)/(TotalSegmentsList{4}*NumGauss),abs(CurrentInterpList{4}),'-c','Linewidth',3);
plot(line_length*(1:TotalSegmentsList{5}*NumGauss)/(TotalSegmentsList{5}*NumGauss),abs(CurrentInterpList{5}),'-m','Linewidth',3);
plot(line_length*(1:TotalSegmentsList{6}*NumGauss)/(TotalSegmentsList{6}*NumGauss),abs(CurrentInterpList{6}),'-k','Linewidth',3);

legend('190 segments','10 segments','40 segments','70 segments','100 segments','130 segments','160 segments');
grid on;xlabel('Segment');ylabel('Magnitude');
set(gca,'fontsize',24);
hold off
subplot(122)
hold on;
plot(line_length*(1:TotalSegmentsList{7}*NumGauss)/(TotalSegmentsList{7}*NumGauss),angle(CurrentInterpList{7})/pi*180,'-y','Linewidth',3);
plot(line_length*(1:TotalSegmentsList{1}*NumGauss)/(TotalSegmentsList{1}*NumGauss),angle(CurrentInterpList{1})/pi*180,'-r','Linewidth',3);
plot(line_length*(1:TotalSegmentsList{2}*NumGauss)/(TotalSegmentsList{2}*NumGauss),angle(CurrentInterpList{2})/pi*180,'-g','Linewidth',3);
plot(line_length*(1:TotalSegmentsList{3}*NumGauss)/(TotalSegmentsList{3}*NumGauss),angle(CurrentInterpList{3})/pi*180,'-b','Linewidth',3);
plot(line_length*(1:TotalSegmentsList{4}*NumGauss)/(TotalSegmentsList{4}*NumGauss),angle(CurrentInterpList{4})/pi*180,'-c','Linewidth',3);
plot(line_length*(1:TotalSegmentsList{5}*NumGauss)/(TotalSegmentsList{5}*NumGauss),angle(CurrentInterpList{5})/pi*180,'-m','Linewidth',3);
plot(line_length*(1:TotalSegmentsList{6}*NumGauss)/(TotalSegmentsList{6}*NumGauss),angle(CurrentInterpList{6})/pi*180,'-k','Linewidth',3);


legend('190 segments','10 segments','40 segments','70 segments','100 segments','130 segments','160 segments');
grid on;xlabel('Segment');ylabel('Phase Angle');
set(gca,'fontsize',24);
hold off
