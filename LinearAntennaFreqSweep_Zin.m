%% sweep the freq in a range of 1MHz-2.5GHz for 1000 freq points
clear all;close all;
Frequency=1e9;
freq_point=1000;
...
%% Freq sweep
Frequency_List= linspace(0.1,5,freq_point)*1e9; % 1MHz-2.5GHz
        ...
        for im = 1:2    % loop testing segments
            ...
            for in=1:2 % loop source segments
                ...
                if ms(im) == ns(in)% self-term
                    ...
                    SegmentNVecN = [ xns(in+1)-xns(in), yns(in+1)-yns(in) ];
...
%% Sweep Zin from solved parameters
% the index of Freq we want to view for the current distribution in Freq_list
[~,FrequencyIdx]=min(abs(Frequency_List-Frequency));
for i=1:freq_point
    [L,U]=lu(A(:,:,i));
    b_temp=U\(L\rhs(:,i));%A(:,:,i)\rhs(:,i);%
    if i==FrequencyIdx
        b=b_temp;
    end
    % compute input impedance at the middle segment
    Zin = Vm / (0.5*(b_temp(SourceId) + b_temp(SourceId+1)));
...
subplot(121)
hold on;
plot(abs(Current),'-r','Linewidth',4);
grid on;xlabel('Segment');ylabel('Magnitude');
title('Current Distribution Numerical Result')
set(gca,'fontsize',24);
hold off
subplot(122)
hold on;
plot(angle(Current)/pi*180,'-b','Linewidth',4);
...
subplot(121)
hold on;
plot(line_length*(1:TotalSegments*NumGauss)/(TotalSegments*NumGauss),abs(CurrentInterp),'-r','Linewidth',4);
...
subplot(122)
hold on;
plot(line_length*(1:TotalSegments*NumGauss)/(TotalSegments*NumGauss),angle(CurrentInterp)/pi*180,'-b','Linewidth',4);
...
figure(4);hold on;
plot(Frequency_List/1e6,real(Zin_NumericalResultList),'-r','Linewidth',4);
plot(Frequency_List/1e6,imag(Zin_NumericalResultList),'-b','Linewidth',4);
grid on;xlabel('Frequency(MHz)');
legend('Resistance=Real[Z_{in}]','Reactance =Imag[Z_{in}]');
title('Input Impedance Numerical Result')
set(gca,'fontsize',24);
hold off